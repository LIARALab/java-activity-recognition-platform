 ...
 */
public class EntityCollectionRestrictionExecutor<T> implements Executor
{
  @NonNull
  private final static String     LIMIT_PARAMETER_NAME  = "first";
  @NonNull
  private final static Pattern    LIMIT_PARAMETER_TEST  = Pattern.compile("^\\s*\\d+\\s*$");
  @NonNull
  private final static String     OFFSET_PARAMETER_NAME = "after";
  @NonNull
  private final static Pattern    OFFSET_PARAMETER_TEST = Pattern.compile("^\\s*\\d+\\s*$");
  @NonNull
  private final static String     ALL_PARAMETER_NAME    = "all";
  @NonNull
  private final static Pattern    ALL_PARAMETER_TEST    = Pattern.compile("^\\s*(true|false|)\\s*$");
  private final static int        DEFAULT_LIMIT         = 10;

  @Nullable
  private EntityCollection<T>     _collection;
  @Nullable
  private EntityCollectionView<T> _result;
  ...
  private
    void
    assertHasValidLimitParameter (@NonNull final List<RequestError> errors, @NonNull final APIRequest request)
  {
    if (request.contains(LIMIT_PARAMETER_NAME)) {
      ...
      if (!LIMIT_PARAMETER_TEST.matcher(request.get(LIMIT_PARAMETER_NAME, 0)).find()) {
        errors.add(this.createLimitParameterContentError(request));
      }
    }
  }

  private RequestError createLimitParameterCountError (@NonNull final APIRequest request) {
    return new RequestParameterError(
      request, LIMIT_PARAMETER_NAME,
      String.join(
        "",
        "Only one \"",
  ...
  private RequestError createLimitParameterContentError (@NonNull final APIRequest request) {
    return new RequestParameterValueError(
      request, LIMIT_PARAMETER_NAME, 0,
      String.join(
        "",
        "\"",
        LIMIT_PARAMETER_NAME,
  ...
  private
    void
    assertHasValidOffsetParameter (@NonNull final List<RequestError> errors, @NonNull final APIRequest request)
  {
    if (request.contains(OFFSET_PARAMETER_NAME)) {
      ...
      if (!OFFSET_PARAMETER_TEST.matcher(request.get(OFFSET_PARAMETER_NAME, 0)).find()) {
        errors.add(this.createOffsetParameterContentError(request));
      }
    }
  }

  private RequestError createOffsetParameterCountError (@NonNull final APIRequest request) {
    return new RequestParameterError(
      request, OFFSET_PARAMETER_NAME,
      String.join(
        "",
        "Only one \"",
  ...
  private RequestError createOffsetParameterContentError (@NonNull final APIRequest request) {
    return new RequestParameterValueError(
      request, OFFSET_PARAMETER_NAME, 0,
      String.join(
        "",
        "\"",
        OFFSET_PARAMETER_NAME,
  ...
  private
    void
    assertHasValidAllParameter (@NonNull final List<RequestError> errors, @NonNull final APIRequest request)
  {
    if (request.contains(ALL_PARAMETER_NAME)) {
      ...
      if (!ALL_PARAMETER_TEST.matcher(request.get(ALL_PARAMETER_NAME, 0)).find()) {
        errors.add(this.createAllParameterContentException(request));
      }
    }
  }

  private RequestError createAllParameterCountException (@NonNull final APIRequest request) {
    return new RequestParameterError(
      request, ALL_PARAMETER_NAME,
      String.join(
        "",
        "Only one \"",
        ALL_PARAMETER_NAME,
  ...
  private RequestError createAllParameterContentException (@NonNull final APIRequest request) {
    return new RequestParameterValueError(
      request, ALL_PARAMETER_NAME, 0,
      String.join(
        "",
        "\"",
        ALL_PARAMETER_NAME,
   ...
   * @param request Request to check.
   */
  private void assertIsValidRequest (@NonNull final List<RequestError> errors, @NonNull final APIRequest request) {
    if (
      request.contains(ALL_PARAMETER_NAME) && request.contains(LIMIT_PARAMETER_NAME)
  ...
  private RequestError createAllAndFirstError (@NonNull final APIRequest request) {
