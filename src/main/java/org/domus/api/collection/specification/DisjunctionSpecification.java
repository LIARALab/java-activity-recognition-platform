...
public class DisjunctionSpecification<Entity> implements Specification<Entity>
{
  @NonNull
  ...
  public DisjunctionSpecification(@NonNull final Iterable<Specification<Entity>> predicates) {
    this._predicates = Lists.newArrayList(predicates);
    this._readOnlyPredicates = Collections.unmodifiableList(this._predicates);
  }

  public DisjunctionSpecification(@NonNull final Iterator<Specification<Entity>> predicates) {
    this._predicates = Lists.newArrayList(predicates);
    this._readOnlyPredicates = Collections.unmodifiableList(this._predicates);
  }

  public DisjunctionSpecification(@NonNull final Specification<Entity>[] predicates) {
    this._predicates = Lists.newArrayList(predicates);
    this._readOnlyPredicates = Collections.unmodifiableList(this._predicates);
  }
